	...
	while (!feof($ponteiro))
		...
		//Novo sistema de contagem de tempo
		$duracao_segundos=($duracao[3]*6);
		$duracao_minutos=($duracao[1].$duracao[2]);
		$duracao_minutos=($duracao_minutos*60);
                $duracao_horas=($duracao[0]*3600);
                
		//Não existe contagem em dias
		$duracao_dias=0;
		...
		if(($duracao)and($time!='::00')and($dataLigacao!='20--')and($dataLigacao<=(date("Y-m-d"))))
			{	
