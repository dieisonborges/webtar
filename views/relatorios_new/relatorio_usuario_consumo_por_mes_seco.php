<?php
		...
		$mes_ano=explode('/', $mes_ano); 
		  ...
		  $rs_usuario = $dalusuario->getTodosSemAdmSemPagConsumo( $mes, $ano, $unidades);
...
-->
  ...
  <div style="position:absolute;top:289;left:303"><span class="ft0">Relat&oacute;rio de Consumo Telefonico</span></div>
  <div style="position:absolute;top:310;left:363"><span class="ft0"><?php echo $mes_ano_original; ?></span></div>
  <div style="position:absolute;top:331;left:10"><span class="ft0">
    <table width="860" border="0" class="tb_relatorio_usuario">
      <tr>
        ...
        <td width="212"><strong>Nome Guerra - SARAM </strong></td>
        <td width="336"><strong>Nome Completo </strong></td>
        <td width="189"><strong>Consumo R$</strong></td>
      </tr>
	  
	  <?php while ($usuario = mysql_fetch_object($rs_usuario)){?>	  
	  <tr>
        <td><?php echo (GetPOSTO($usuario->postoGraduacao)); ?></td>
        <td><?php echo $usuario->nomeGuerra; ?>-<?php echo $usuario->saram; ?></td>
        <td><?php echo $usuario->nomeCompleto; ?></td>
        <td><?php echo round($usuario->valor,2); ?></td>
      </tr>
     <?php }?>
