...
public function getTodosSemAdmSemPagConsumo( $mes, $ano, $tbUnidades_id)
    {
        $this->conexao->AbreConexao();
        $this->conexao->SelecionaBanco();
		$sql = "select sum(lig.valor) as valor, count(lig.id) as qtd_ligacoes, usu.usuario as usuario, usu.nomeCompleto as nomeCompleto, usu.nomeGuerra as nomeGuerra, usu.saram as saram, usu.postoGraduacao as postoGraduacao, lig.status_gestor as status_gestor from  tbLigacoes as lig inner join tbUsuario as usu on usu.id=lig.tbUsuario_id
						where lig.tbUnidades_id='$tbUnidades_id' and
                                                year(lig.dataLigacao)='$ano' and 
						month (lig.dataLigacao)='$mes' 
						group by usu.id
